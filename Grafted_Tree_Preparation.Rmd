...
existing_tree <- read.tree("/Users/laurelhiebert/Documents/06. Project Scientist Yi Lab/04. Review MS/Phylogenetic_comparative_analysis/Scaled_tree.nwk")
...
if (is.ultrametric(tree_ultrametric)) {
...
# 4.2 Suppose you’ve already built these vectors (one for each rank),
#     each length == tree$Nnode, storing the label (or "") for that node.
#     For example:
#       genus_labels  <- label_nodes_by_rank(tree, taxonomy_df, "Genus")
#       family_labels <- label_nodes_by_rank(tree, taxonomy_df, "Family")
#       order_labels  <- label_nodes_by_rank(tree, taxonomy_df, "Order")
#       class_labels  <- label_nodes_by_rank(tree, taxonomy_df, "Class")
#       phylum_labels <- label_nodes_by_rank(tree, taxonomy_df, "Phylum")

# For demonstration, assume you’ve combined them into a data frame
# called node_label_data:
# node_label_data <- data.frame(
#   Genus  = genus_labels,
#   Family = family_labels,
#   Order  = order_labels,
...
full_topology_tree <- read.tree("/Users/laurelhiebert/Documents/06. Project Scientist Yi Lab/04. Review MS/Phylogenetic_comparative_analysis/Full_topology.nwk")
...
# distances are equal. This can change the old branch lengths, so do it only if
# it's acceptable in your pipeline.
...
cat("Saved final tree with new species to 'final_grafted_tree.newick'.\n")

# Quick visualization with ggtree
ggtree(updated_tree) + 
  geom_tiplab(size = 2) +
  ggtitle("Final Grafted Tree")
```
