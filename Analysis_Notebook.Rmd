...
tree_grafted_file <- "/Users/laurelhiebert/Documents/10. Project Scientist Yi Lab/04. Review MS/Phylogenetic_comparative_analysis/GPT/phylogenetic_tree_grafted.newick"
...
loginfo("Loading phylogenetic tree from: %s", tree_grafted_file)
...
required_cols <- c("Species", "Methylation", "Germline_Timing", "Regen_Ability")
...
clean_and_match_data <- function(tree, traits_original) {
  ...
  if(!is.binary(tree_pruned)) {
    ...
    tree_pruned <- phytools::multi2di(tree_pruned)
...
# Save
ggsave("PNAS_Style_Regeneration_Methylation_Plot.pdf", p, width = 7.5, height = 6, dpi = 600)
...
model_names <- c("lambda_Regen", "lambda_Multi",
                 "BM_Regen", "BM_Null",
                 "OUrandomRoot_Regen", "OUrandomRoot_Null",
                 "kappa_Regen", "kappa_Null",
                 "delta_Regen", "delta_Null",
                 "EB_Regen", "EB_Null")
...
## Step 11: Jackknife Leave-One-Out Sensitivity Analysis for the lambda_Regen Model

```{r}
###############################################################################
# Step 11: Jackknife Leave-One-Out Sensitivity Analysis for the lambda_Multi Model
###############################################################################

# 1. Identify Inputs (already in your workspace)
tree_for_jackknife <- tree_ultrametric         # or tree_pruned if that's your final tree
lambda_model       <- fitted_models$lambda_Regen
